...
## Afgerond
**"AFSPRAKEN"-toggle in de agendakolom** — KLAAR (commit "AFSPRAKEN-toggle in agendakolom afgemaakt").
- `afsprakenGroupsHTML()` is gedeelde lijst-builder (gebruikt door `renderAfspraken()` én `renderAgenda()`).
- `renderAgenda()` toont bij `agendaCol==='afspraken'` de lijst i.p.v. de kalender en zet de class `afsprakenmode` op `#view-agenda` → ::before-label wordt "Afspraken".
- `toggleBtn` wordt nu in BEIDE takken gerenderd (was de bug: ontbrak in de kalender-tak, dus vanuit de kalender kon je niet naar AFSPRAKEN schakelen). CSS: `.agenda-toggle` (tekstknop, zelfde font als kolomlabels, links naast de +-knop).

## Openstaande TODO
Geen — de vorige openstaande taak (AFSPRAKEN-toggle) is afgerond.
